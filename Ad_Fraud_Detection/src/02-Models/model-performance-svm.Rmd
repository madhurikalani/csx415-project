...
model_svm <- train(as.factor(is_attributed)~app + channel + ip +device + os + click_time, data=df.train, method="rf", trControl=train_control)

```
### Summary of Trained SVM Model
...
```
### Fitting the Trained SVM Model
```{r p12,echo =FALSE , warning=FALSE}
predict_svm <- predict(model_svm, df.test)
```
 ## Evaluation 
...
cm_svm <- confusionMatrix(table(as.factor(df.test$is_attributed),predict_svm))
cm_svm
```
### Step 2 Finding Cohen's Kappa

```{r p14,echo =FALSE , warning=FALSE}
print("kappa value for SVM")
cm_svm$overall[2]
```

### Step 3 Finding Accuracy Value
```{r p15,echo =FALSE , warning=FALSE}
print("Accuracy value for SVM")
cm_svm$overall[1]
```
...
library(pROC)
plot(roc(df.test$is_attributed, as.numeric(predict_svm), direction="<"),
      col="yellow", lwd=3, main="ROC Curve for SVM")
